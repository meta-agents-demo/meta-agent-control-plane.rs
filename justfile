...
production-workers-up profile="prod" acknowledgment="":
    ...
    docker compose \
      ...
      -f compose.agents.yaml \
      -f compose.production.yaml \
      up --detach --build control-plane agent-runner-openai agent-runner-anthropic
...
production-down profile="prod":
    ...
    compose_env="env/dec/runtime-secrets/{{ profile }}/compose.env"; \
      if test -f "$compose_env"; then \
        docker compose --profile production-workers --profile production-mutation --env-file "$compose_env" -f compose.agents.yaml -f compose.production.yaml down --remove-orphans; \
      fi
